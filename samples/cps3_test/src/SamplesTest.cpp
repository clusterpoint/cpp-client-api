...
void SamplesTest::run_tests() {
	RUN_TEST(test_insert_sample); clear();
	RUN_TEST(test_update_sample); clear();
	RUN_TEST(test_replace_sample); clear();
	RUN_TEST(test_partial_replace_sample); clear();
	RUN_TEST(test_delete_sample); clear();
	RUN_TEST(test_search_sample); clear();
	RUN_TEST(test_lookup_sample); clear();
	RUN_TEST(test_retrieve_sample); clear();
	RUN_TEST(test_list_last_sample); clear();
	RUN_TEST(test_list_paths_sample); clear();
	RUN_TEST(test_status_sample); clear();
	RUN_TEST(test_clear_sample); clear();
	RUN_TEST(test_reindex_sample); clear();
}
...
void SamplesTest::test_insert_sample() {
	try
	{
		...
		account_info["account"] = "YOUR_ACCOUNT_ID";
		CPS::Connection *conn = new CPS::Connection("tcp://cloud-eu-0.clusterpoint.com:9007", "DB_NAME", "USERNAME", "PASSWORD", "document", "document/id", account_info);
#endif
...
void SamplesTest::test_replace_sample() {
	try
	{
		...
		CPS::ReplaceResponse *replace_resp = conn->sendRequest<CPS::ReplaceResponse>(replace_req);
		// Print out modified ids
...
void SamplesTest::test_partial_replace_sample() {
	try
	{
		...
		CPS::PartialReplaceResponse *partial_replace_resp = conn->sendRequest<CPS::UpdateResponse>(partial_replace_req);
		// Print out modified ids
		std::cout << "Partial replaced ids: " << CPS::Utils::join(partial_replace_resp->getModifiedIds()) << std::endl;

		// Clean Up
		delete insert_resp;
		delete partial_replace_resp;
#if 0
		delete conn;
#endif
	}
